// Build a self-contained visual gallery from a GREENFIELD (new-org) walkthrough run.
// Reads shots/neworg-results.json + shots/wt-*.png (written by walkthrough.spec.ts),
// compresses each screenshot (macOS `sips`), and emits neworg-gallery.html with
// images embedded as base64. Open in a browser or publish as an artifact.
//
//   node build-neworg-gallery.mjs    # after: npx playwright test walkthrough-neworg.spec.ts
...
const steps = JSON.parse(readFileSync(join(HERE, 'shots', 'neworg-results.json'), 'utf8'));
...
const FLOWS = {
  org: {
    label: 'Act I · A hospital is born',
    persona: 'PLATFORM ADMIN',
    blurb: 'Meridian Care Hospital is registered through the real 4-step wizard, then fully provisioned: every feature module, starter departments/wards/drug catalog, six staff logins, a registry-verified doctor with a weekly schedule, and pharmacy stock.',
  },
  clinical: {
    label: 'Act II · The first patient',
    persona: 'RECEPTION → NURSE → DOCTOR → LAB → PHARMACY → BILLING',
    blurb: 'One patient, end to end, entirely in the new hospital: registration, appointment, triage, consultation with an inline lab order and a prescription, the full lab workflow, pharmacy dispensing, and a settled invoice.',
  },
};
...
const html = `<title>Meridian Care — From Zero to First Patient</title>
<style>
...
<div class="wrap">
  <header class="top">
    <div class="brand"><div class="logo">✚</div><div><div class="eyebrow">Ayphen HMS · Live UI walkthrough</div><h1>A hospital from zero to first patient, live</h1></div></div>
    <div class="score"><b>${pass} / ${steps.length}</b><span>steps passed</span></div>
  </header>
  <p class="lede">A brand-new organization was created on the production SaaS and taken all the way to its first settled invoice — every screen below driven in a real browser, every outcome confirmed against the API. The hospital persists afterwards as a fully-working demo org.</p>
  <div class="legend"><span><b>UI-driven</b> — real browser &amp; logins</span><span><b>API-verified</b> — outcomes checked server-side</span><span><b>Persistent</b> — the org remains as a demo hospital</span><span><b>Environment</b> — care-sims.vercel.app · live</span></div>
  <div class="glance">
${Object.entries(FLOWS).map(([k, f]) => `    <div class="glance-col"><h4>${esc(f.label)}</h4><ol>${(glance[k] || []).map(t => `<li>${esc(t)}</li>`).join('')}</ol></div>`).join('\n')}
  </div>
${sections}
  <footer><span><b>Run:</b> ${steps.length} steps · ${pass}/${steps.length} PASS</span><span><b>Spec:</b> ayphen-hms/e2e/walkthrough-neworg.spec.ts</span><span><b>Re-run:</b> npx playwright test walkthrough-neworg.spec.ts &amp;&amp; node build-neworg-gallery.mjs</span></footer>
</div>`;

writeFileSync(join(HERE, 'neworg-gallery.html'), html);
console.log(`neworg-gallery.html written — ${(html.length / 1024 / 1024).toFixed(2)} MB · ${pass}/${steps.length} PASS`);
