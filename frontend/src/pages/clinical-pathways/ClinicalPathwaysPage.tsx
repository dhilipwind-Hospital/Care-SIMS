  ...
  const fetchData = async () => {
    ...
    try {
      const [list, dash] = await Promise.all([
        api.get('/clinical-pathways', { params: { page, limit: 20 } }),
        api.get('/clinical-pathways/dashboard').catch(() => ({ data: {} })),
      ]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <KpiCard label="Total Records" value={dashboard.total || records.length || 0} icon={GitBranch} color="#0F766E" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              : records.length === 0 ? <tr><td colSpan={4}><EmptyState icon={<GitBranch size={36} />} title="No records yet" description="Records will appear here when created" /></td></tr>
              : records.map((r: any) => (
