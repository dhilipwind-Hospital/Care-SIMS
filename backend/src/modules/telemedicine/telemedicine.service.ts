...
export class TelemedicineService {
  ...
  async start(tenantId: string, id: string) { const session = await this.prisma.teleconsultSession.findFirst({ where: { id, tenantId } }); if (!session) throw new NotFoundException('Session not found'); return this.prisma.teleconsultSession.update({ where: { id, tenantId }, data: { startedAt: new Date(), status: 'IN_PROGRESS' } }); }
  async end(tenantId: string, id: string) { const s = await this.prisma.teleconsultSession.findFirst({ where: { id, tenantId } }); if (!s) throw new NotFoundException('Session not found'); const dur = s?.startedAt ? Math.round((Date.now() - s.startedAt.getTime()) / 60000) : 0; return this.prisma.teleconsultSession.update({ where: { id, tenantId }, data: { endedAt: new Date(), durationMinutes: dur, status: 'COMPLETED' } }); }
  async cancel(tenantId: string, id: string, reason: string) { const session = await this.prisma.teleconsultSession.findFirst({ where: { id, tenantId } }); if (!session) throw new NotFoundException('Session not found'); return this.prisma.teleconsultSession.update({ where: { id, tenantId }, data: { status: 'CANCELLED', cancelledReason: reason } }); }
