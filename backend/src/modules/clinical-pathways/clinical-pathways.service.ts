...
export class ClinicalPathwaysService {
  ...
  async assignPathway(tenantId: string, dto: any) { if (!dto.patientId || !dto.protocolId) throw new BadRequestException('patientId and protocolId are required'); return this.prisma.patientPathway.create({ data: { tenantId, protocolId: dto.protocolId, patientId: dto.patientId, admissionId: dto.admissionId, notes: dto.notes } }); }
